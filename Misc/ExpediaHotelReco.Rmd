...
## Data Files
The following are the data files provided.
...
The **training** dataset has 37M records with 4GB in size. we can't load the complete training datset in a normal computer. Need a better machine with atleast 16GB RAM size.

The following are some ways to deal with big data set.
...
## Does the destination id specified in the **test** data present in the training data?

```{r}
# get the size of the destination id list for test data
test_dest_list_len <- length(unique(test_dt$srch_destination_id))
test_dest_list_len

intersect_list_len <- length(intersect(test_dt$srch_destination_id, train_dt$srch_destination_id))
intersect_list_len

test_dest_list_len == intersect_list_len
```

## Charts


### The user's current location and the destination hotel location
